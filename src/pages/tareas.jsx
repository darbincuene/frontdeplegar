...
function Tareas() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:12345/tarea")
      .then((response) => {
  ...
  const eliminarTarea = (id) => {
    axios
      .delete(`http://localhost:12345/tarea/${id}`)
      .then(() => {
    ...
    <>
      <div class="py-8 px-8">
        <button
          ...
          className="bg-blue-800 hover:bg-green-700 text-white  px-4 rounded-3xl cursor-pointer"
        >
          Crear tarea
        </button>
        ...
        <div className="grid grid-cols-3 gap-4 ">
          {data.map((tarea) => (
            <div key={tarea.id} className="bg-amber-950 px-8 py-8 rounded-3xl">
              <h2 className="flex justify-center px-4 py-4 text-white">
                {tarea.titulo}
              </h2>
              <p className="flex justify-center px-4 py-4 text-white ">
              ...
              <div className="flex justify-center gap-4">
                <button
                  ...
                  className="bg-red-200  px-4 rounded-3xl cursor-pointer"
                >
                  Eliminar
                </button>
                <button
