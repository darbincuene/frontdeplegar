...
function Tarea() {
  ...
  useEffect(() => {
    axios.get("https://tareasdarbin.onrender.com/tareas")
        ...
        <div className="grid grid-cols-3 gap-4">
          {datos.map((tarea) => (
            <div key={tarea.id} className="bg-amber-950 px-8 py-8 rounded-3xl">
              ...
              <p className="text-center px-4 py-4 text-white">{tarea.descripcion ?? "Sin descripción"}</p>
              <p className="text-center px-4 py-4 text-white">{tarea.completada ? "Completada" : "Pendiente"}</p>
